...
async function trainVisualFiles(files,kind){
  const item=kind==='item',limit=item?6:4,list=[...files].slice(0,limit),generation=item?itemEditGeneration:locationEditGeneration,entityId=item?$('#itemForm').elements.id.value:$('#locationForm').elements.id.value;if(!list.length)return;
  ...
  setMediaPending(kind,generation,1);
  try{for(const file of list){const sample=await createVisualFingerprint(file),stale=item?(generation!==itemEditGeneration||!$('#itemDialog').open||$('#itemForm').elements.id.value!==entityId):(generation!==locationEditGeneration||!$('#locationDialog').open||$('#locationForm').elements.id.value!==entityId);if(stale)return;if(item)pendingItemVisualSamples=appendUniqueVisualSample(pendingItemVisualSamples,sample,limit);else pendingLocationVisualSamples=appendUniqueVisualSample(pendingLocationVisualSamples,sample,limit);}updateVisualTrainingStatus(kind);toast('Fotoerkennung lokal angelernt');}
  catch(error){console.error(error);const active=item?(generation===itemEditGeneration&&$('#itemDialog').open&&$('#itemForm').elements.id.value===entityId):(generation===locationEditGeneration&&$('#locationDialog').open&&$('#locationForm').elements.id.value===entityId);if(active)status.textContent='Mindestens ein Bild konnte nicht ausgewertet werden. Bitte erneut fotografieren.';}
  finally{setMediaPending(kind,generation,-1);}
}
...
function findKnownCode(text,exactOnly=false){
  const raw=String(text||''),compact=compactCode(raw),tokens=raw.split(/[\s,;:()[\]{}<>|]+/).map(compactCode).filter(Boolean),entries=[...state.items.filter(item=>!item.archived).flatMap(item=>[{kind:'item',entity:item,code:item.barcode},{kind:'item',entity:item,code:item.sku}]),...(state.locations||[]).map(entity=>({kind:'location',entity,code:entity.code}))].filter(entry=>compactCode(entry.code).length>=4).sort((a,b)=>String(b.code).length-String(a.code).length);
...
function findArchivedKnownCode(text,exactOnly=false){
  const raw=String(text||''),compact=compactCode(raw),tokens=raw.split(/[\s,;:()[\]{}<>|]+/).map(compactCode).filter(Boolean),entries=state.items.filter(item=>item.archived).flatMap(item=>[{kind:'item',entity:item,code:item.barcode},{kind:'item',entity:item,code:item.sku}]).filter(entry=>compactCode(entry.code).length>=4);
  const matches=entries.filter(entry=>{const code=compactCode(entry.code);if(exactOnly)return compact===code;return tokens.includes(code)||(code.length>=8&&compact.includes(code));});const unique=[...new Map(matches.map(entry=>[entry.entity.id,entry])).values()];return unique.length===1?unique[0]:null;
}
...
function loadTesseractLocal(){
  ...
  window.__hp67TesseractPromise=withTimeout(new Promise((resolve,reject)=>{const script=document.createElement('script');script.src='./vendor/tesseract/tesseract.min.js';script.onload=()=>window.Tesseract?resolve(window.Tesseract):reject(new Error('Lokale Texterkennung ist unvollständig.'));script.onerror=()=>reject(new Error('Lokale Texterkennung konnte nicht geladen werden.'));document.head.append(script);}),15000,'Texterkennung reagiert nicht').catch(error=>{window.__hp67TesseractPromise=null;throw error;});return window.__hp67TesseractPromise;
}
async function recognizeImageLocally(image,languages,logger){
  const api=await loadTesseractLocal(),root=new URL('./vendor/tesseract/',location.href),worker=await api.createWorker(Array.isArray(languages)?languages:[languages],1,{workerPath:new URL('worker.min.js',root).href,corePath:new URL('core/',root).href,langPath:new URL('lang',root).href,workerBlobURL:false,logger});
  try{return await worker.recognize(image);}finally{try{await worker.terminate();}catch{}}
}
...
async function analyzeSmartPhoto(file){
  ...
  try{
    ...
    barcode=await scanBarcodeFromFile(file,run);if(!smartRunActive(run))return;if(barcode){const exact=findKnownCode(barcode,true),archived=findArchivedKnownCode(barcode,true);if(exact)smartCandidates.push({...exact,source:'barcode',confidence:1});else if(archived){smartCandidates.push({...archived,source:'barcode',confidence:1,blocked:true,archived:true});notes.push({type:'warning',title:'Artikel ist archiviert',text:'Zum Buchen bitte den Treffer öffnen und den Artikel zuerst reaktivieren.'});}else{smartUnknownCode=barcode;notes.push({type:'warning',title:'Neuer Barcode erkannt',text:`${barcode} ist noch nicht zugeordnet. Fotoähnlichkeiten werden nur als Hinweis gezeigt und können nicht gebucht werden.`});}}
    ...
    if(exactBarcode){smartNewSuggestion={barcode:smartUnknownCode};setSmartProgress(88,'Bekannter Barcode eindeutig zugeordnet.',run);}
      ...
      try{await loadTesseractLocal();if(!smartRunActive(run))return;const result=await withTimeout(recognizeImageLocally(analysisFile,['deu','eng'],message=>{if(message.status==='recognizing text')setSmartProgress(63+Math.round((message.progress||0)*25),`Text wird lokal erkannt … ${Math.round((message.progress||0)*100)} %`,run);}),45000,'Texterkennung nach 45 Sekunden beendet');if(!smartRunActive(run))return;ocrText=String(result?.data?.text||'');const exact=findKnownCode(ocrText),archived=!exact?findArchivedKnownCode(ocrText):null;if(exact&&!smartCandidates.some(candidate=>candidate.kind===exact.kind&&candidate.entity.id===exact.entity.id))smartCandidates.unshift({...exact,source:'ocr',confidence:.96,blocked:!!smartUnknownCode});else if(archived&&!smartCandidates.some(candidate=>candidate.entity.id===archived.entity.id))smartCandidates.unshift({...archived,source:'ocr',confidence:.96,blocked:true,archived:true});smartNewSuggestion={...suggestionFromText(ocrText),barcode:smartUnknownCode};}
      catch(error){console.warn(error);smartNewSuggestion={barcode:smartUnknownCode};notes.push({title:'Lokale Texterkennung nicht verfügbar',text:'Barcode- und Fotovergleich funktionieren trotzdem. Unter „Mehr → App aktualisieren“ kann das lokale OCR-Paket erneut geladen werden.'});}
    }
...
function closeSmartDialog(after=null){requestDialogClose($('#smartCameraDialog'),after,{skipDirty:true});}
function showSmartLocation(location){closeSmartDialog(()=>openInventoryLocation(location.id));}
$('#smartOpenResult').onclick=()=>{const candidate=currentSmartCandidate();if(!candidate)return;closeSmartDialog(()=>candidate.kind==='item'?openItem(candidate.entity.id):openInventoryLocation(candidate.entity.id));};
...
function bookSmartCandidate(type){const candidate=currentSmartCandidate();if(candidate?.kind!=='item'||candidate.entity.archived||candidate.blocked||candidate.ambiguous)return;const item=candidate.entity,needsConfirmation=candidate.source!=='barcode';if(needsConfirmation&&!confirm(`${candidate.source==='photo'?'Foto-Treffer':'Code aus Etikettentext'} „${item.name}“${candidate.source==='photo'?` (${Math.round(candidate.confidence*100)} %)`:''} wirklich bestätigen und 1 Stück ${type==='purchase'?'einbuchen':'ausbuchen'}?`))return;if(type==='sale'){if(item.stock<=0){toast('Artikel ist bereits ausverkauft');return;}closeSmartDialog(()=>openTransaction(item.id,'sale'));return;}const before=item.stock;item.stock+=1;state.transactions.push({id:uid(),itemId:item.id,itemName:item.name,type:'purchase',quantity:1,unitPrice:item.cost,unitCost:item.cost,stockBefore:before,stockAfter:item.stock,note:`Smart-Kamera · ${candidate.source==='barcode'?'Barcode exakt':candidate.source==='photo'?'Foto bestätigt':'Textcode bestätigt'}`,date:new Date().toISOString()});save();closeSmartDialog();toast('+1 per Smart-Kamera eingebucht');}
