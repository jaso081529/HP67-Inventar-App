...
function loadState(){
  try{
    ...
    if(saved && Array.isArray(saved.items) && Array.isArray(saved.transactions)){saved.orders=Array.isArray(saved.orders)?saved.orders:[];saved.snapshots=Array.isArray(saved.snapshots)?saved.snapshots:[];saved.settings=saved.settings&&typeof saved.settings==='object'?saved.settings:{};saved.items.forEach(i=>{if(typeof i.archived!=='boolean')i.archived=false;});saved.categories=Array.isArray(saved.categories)?saved.categories:DEFAULT_CATEGORIES.map(c=>({...c}));DEFAULT_CATEGORIES.forEach(def=>{if(!saved.categories.some(c=>c.name.toLowerCase()===def.name.toLowerCase()))saved.categories.push({...def});});const known=new Set(saved.categories.map(c=>String(c.name).toLowerCase()));saved.items.map(i=>String(i.category||'').trim()).filter(Boolean).forEach(name=>{if(!known.has(name.toLowerCase())){saved.categories.push({id:`cat-${Date.now()}-${saved.categories.length}`,name,prefix:'',color:'#64748b'});known.add(name.toLowerCase());}});saved.brands=Array.isArray(saved.brands)?saved.brands:DEFAULT_BRANDS.slice();DEFAULT_BRANDS.forEach(brand=>{if(!saved.brands.some(b=>b.toLowerCase()===brand.toLowerCase()))saved.brands.push(brand);});saved.items.map(i=>String(i.brand||'').trim()).filter(Boolean).forEach(brand=>{if(!saved.brands.some(b=>b.toLowerCase()===brand.toLowerCase()))saved.brands.push(brand);});saved.materials=Array.isArray(saved.materials)?saved.materials:DEFAULT_MATERIALS.slice();DEFAULT_MATERIALS.forEach(material=>{if(!saved.materials.some(m=>m.toLowerCase()===material.toLowerCase()))saved.materials.push(material);});saved.items.map(i=>String(i.material||'').trim()).filter(Boolean).forEach(material=>{if(!saved.materials.some(m=>m.toLowerCase()===material.toLowerCase()))saved.materials.push(material);});return saved;}
...
function renderSalesChart(){
  ...
  $('#salesChart').innerHTML=months.map(m=>`<div class="chart-month" title="${esc(m.label)}: ${money(m.sales)} Umsatz, ${money(m.profit)} Rohertrag"><div class="chart-bars"><span class="chart-bar" style="height:${Math.max(3,m.sales/max*100)}%"></span><span class="chart-bar profit" style="height:${Math.max(3,m.profit/max*100)}%"></span></div><small>${esc(m.label)}</small></div>`).join('');
}
function itemCard(i){
...
function renderInventory(){
  ...
  const items=state.items.filter(i=>matchesFilter(i)&&(!q||[i.name,i.sku,i.category,i.color,i.size,i.barcode,i.supplier,i.location,i.brand,i.material,i.season,i.tags,i.description].join(' ').toLowerCase().includes(q))).sort((a,b)=>inventorySort==='stock-asc'?a.stock-b.stock:inventorySort==='stock-desc'?b.stock-a.stock:inventorySort==='value-desc'?(b.stock*b.cost)-(a.stock*a.cost):a.name.localeCompare(b.name,'de'));
...
function renderTransactions(){
  ...
  $('#activityList').innerHTML=list.length?list.map(transactionCard).join(''):empty('Für diesen Filter gibt es noch keine Buchungen.');
...
$('#createSnapshot').onclick=()=>{const active=state.items.filter(i=>!i.archived),snapshot={id:uid(),date:new Date().toISOString(),itemCount:active.length,totalUnits:active.reduce((s,i)=>s+i.stock,0),value:active.reduce((s,i)=>s+i.stock*i.cost,0),items:active.map(i=>({id:i.id,name:i.name,sku:i.sku,stock:i.stock,cost:i.cost,value:i.stock*i.cost}))};state.snapshots.push(snapshot);if(state.snapshots.length>24)state.snapshots=state.snapshots.slice(-24);save();toast('Inventar-Momentaufnahme gespeichert');};
function fillItemSelect(){
  const select=$('#transactionItem'),current=select.value;
  const active=state.items.filter(i=>!i.archived);select.innerHTML=active.length?active.slice().sort((a,b)=>a.name.localeCompare(b.name,'de')).map(i=>`<option value="${esc(i.id)}">${esc(i.name)} (${number(i.stock)})</option>`).join(''):'<option value="">Zuerst Artikel anlegen</option>';
...
function openItem(id){
  ...
  pendingItemPhoto=item?.photo||''; $('#itemPhotoPreview').src=pendingItemPhoto||'icon.svg'; $('#removeItemPhoto').hidden=!pendingItemPhoto; $('#itemPhoto').value='';
...
function openTransaction(itemId=''){
  ...
  const f=$('#transactionForm'); f.reset(); fillItemSelect(); if(itemId) f.elements.itemId.value=itemId; const now=new Date(); f.elements.date.value=new Date(now-now.getTimezoneOffset()*60000).toISOString().slice(0,16); $('#transactionDialog').showModal();
}
...
const TEXTILE_TYPES=[
  {code:'TS',label:'T-Shirts',match:/\b(t[ -]?shirt|tee)\b/i},{code:'HD',label:'Hoodies',match:/\b(hoodie|kapuzenpullover|kapuzenjacke)\b/i},
  ...
  {code:'PO',label:'Poloshirts',match:/\b(polo|poloshirt)\b/i},{code:'HM',label:'Hemden',match:/\b(hemd|shirt)\b/i},
...
$('#transactionForm').addEventListener('submit',e=>{
  e.preventDefault(); const fd=new FormData(e.currentTarget),item=state.items.find(i=>i.id===fd.get('itemId')),type=fd.get('type'),qty=+fd.get('quantity'); if(!item)return;
  if((type==='sale'||type==='damage')&&qty>item.stock&&!confirm(`Nur ${item.stock} Stück verfügbar. Trotzdem ${qty} Stück ausbuchen?`))return;
  ...
  const unitPrice=+fd.get('unitPrice')||((type==='sale'||type==='return')?item.salePrice:item.cost);
  const unitCost=item.cost;
  if(type==='purchase'&&unitPrice>0)item.cost=unitPrice;
  const chosenDate=fd.get('date');
  state.transactions.push({id:uid(),itemId:item.id,itemName:item.name,type,quantity:qty,unitPrice,unitCost,stockBefore,stockAfter:item.stock,note:fd.get('note').trim(),date:chosenDate?new Date(chosenDate).toISOString():new Date().toISOString()});
  save(); $('#transactionDialog').close(); toast('Buchung gespeichert');
});
...
$('#quickPurchase').onclick=()=>{openTransaction();if(state.items.length)$('#transactionForm').elements.type.value='purchase';};
$('#quickSale').onclick=()=>{openTransaction();if(state.items.length)$('#transactionForm').elements.type.value='sale';};
...
$$('[data-activity]').forEach(b=>b.onclick=()=>{activityFilter=b.dataset.activity;$$('[data-activity]').forEach(x=>x.classList.toggle('active',x===b));renderTransactions();});
...
function openReceive(orderId){
  ...
  $('#receiveRows').innerHTML=order.lines.map(line=>{const remaining=Math.max(0,line.quantity-(line.received||0));return `<div class="planning-row" data-receive="${esc(line.itemId)}"><span><b>${esc(line.itemName)}</b><small>Bestellt ${line.quantity} · erhalten ${line.received||0} · offen ${remaining}</small></span><input type="number" min="0" max="${remaining}" step="1" value="${remaining}" aria-label="Eingang ${esc(line.itemName)}"></div>`;}).join('');$('#receiveDialog').showModal();
}
$('#receiveForm').addEventListener('submit',e=>{
  ...
  $$('[data-receive]','#receiveRows').forEach(row=>{const line=order.lines.find(l=>l.itemId===row.dataset.receive),qty=+$('input',row).value,remaining=line?line.quantity-(line.received||0):0;if(!line||qty<=0)return;if(qty>remaining){alert(`Bei ${line.itemName} sind nur ${remaining} Stück offen.`);return;}const item=state.items.find(i=>i.id===line.itemId);if(!item)return;const stockBefore=item.stock;item.stock+=qty;line.received=(line.received||0)+qty;state.transactions.push({id:uid(),itemId:item.id,itemName:item.name,type:'purchase',quantity:qty,unitPrice:line.unitCost,unitCost:item.cost,stockBefore,stockAfter:item.stock,note:`Wareneingang · ${order.supplier}`,date});receivedCount+=qty;});
...
$('#voiceForm').addEventListener('submit',e=>{
  ...
  const resolved=rows.map(row=>({row,item:state.items.find(i=>i.name.localeCompare(row.name,'de',{sensitivity:'base'})===0||i.sku?.toLowerCase()===row.name.toLowerCase()||i.barcode===row.name)}));const unknown=resolved.filter(x=>!x.item&&mode!=='purchase');if(unknown.length){alert(`Nicht im Inventar gefunden: ${unknown.map(x=>x.row.name).join(', ')}. Bitte zuerst als Einkauf oder Artikel anlegen.`);return;}const shortages=resolved.filter(x=>x.item&&(mode==='sale'||mode==='damage')&&x.row.quantity>x.item.stock);if(shortages.length&&!confirm(`Bei ${shortages.map(x=>x.row.name).join(', ')} reicht der Bestand nicht aus. Trotzdem bis auf 0 ausbuchen?`))return;
  const date=new Date().toISOString();resolved.forEach(({row,item})=>{if(!item){item={id:uid(),name:row.name.charAt(0).toUpperCase()+row.name.slice(1),sku:'',category:'',color:'',size:'',supplier:'',location:'',barcode:'',photo:'',stock:0,minStock:5,cost:row.price,salePrice:0};state.items.push(item);}const stockBefore=item.stock,unitCost=item.cost;if(mode==='purchase'||mode==='return')item.stock+=row.quantity;else item.stock=Math.max(0,item.stock-row.quantity);const price=row.price||((mode==='sale'||mode==='return')?item.salePrice:item.cost);if(mode==='purchase'&&row.price>0)item.cost=row.price;state.transactions.push({id:uid(),itemId:item.id,itemName:item.name,type:mode,quantity:row.quantity,unitPrice:price,unitCost,stockBefore,stockAfter:item.stock,note:'Spracheingabe',date});});save();$('#voiceDialog').close();toast(`${rows.length} Positionen per Sprache verbucht`);
});
...
$('#cameraInput').onchange=$('#galleryInput').onchange=e=>processReceipt(e.target.files[0]);
async function processReceipt(file){
  if(!file)return; navigate('scan'); const url=URL.createObjectURL(file); $('#receiptPreview').src=url; $('#receiptPreview').style.display='block'; showScanRows([], 'Texterkennung wird vorbereitet …');
  try{
    if(!window.Tesseract){
      await new Promise((resolve,reject)=>{const s=document.createElement('script');s.src='https://cdn.jsdelivr.net/npm/tesseract.js@5/dist/tesseract.min.js';s.onload=resolve;s.onerror=reject;document.head.append(s);});
    }
    const result=await Tesseract.recognize(file,'deu',{logger:m=>{if(m.status==='recognizing text')$('#ocrStatus').textContent=`Text wird erkannt … ${Math.round((m.progress||0)*100)} %`;}});
  ...
  }catch(err){ $('#ocrStatus').textContent='Die automatische Erkennung konnte nicht geladen werden. Bitte Positionen manuell eingeben; hierfür ist keine Internetverbindung nötig.'; console.error(err); }
}
$('#scanForm').addEventListener('submit',e=>{
  ...
  rows.forEach(r=>{
    let item=state.items.find(i=>i.name.localeCompare(r.name,'de',{sensitivity:'base'})===0);
    if(!item){item={id:uid(),name:r.name,sku:'',category:'',color:'',size:'',supplier:'',location:'',barcode:'',stock:0,minStock:5,cost:r.cost,salePrice:0};state.items.push(item);}
    const stockBefore=item.stock,unitCost=item.cost;item.stock+=r.quantity;if(r.cost>0)item.cost=r.cost;
    state.transactions.push({id:uid(),itemId:item.id,itemName:item.name,type:'purchase',quantity:r.quantity,unitPrice:r.cost,unitCost,stockBefore,stockAfter:item.stock,note:'Belegimport',date:new Date().toISOString()});
  });
  save(); $('#scanDialog').close(); $('#receiptPreview').style.display='none'; navigate('inventory'); toast(`${rows.length} Positionen eingebucht`);
});

async function stopBarcodeScanner(){if(barcodeScanner){try{if(barcodeScanner.isScanning)await barcodeScanner.stop();await barcodeScanner.clear();}catch{}barcodeScanner=null;}}
function openBarcodeScanner(target='lookup'){
  scannerTarget=target;scannedItem=null;$('#manualBarcode').value='';$('#scannerResult').hidden=true;$('#scannerActions').hidden=true;$('#scannerStatus').textContent='Kamera wird vorbereitet …';$('#scannerDialog').showModal();
  setTimeout(startBarcodeScanner,80);
}
async function startBarcodeScanner(){
  if(typeof Html5Qrcode==='undefined'){$('#scannerStatus').textContent='Scanner-Modul nicht geladen. Code bitte manuell eingeben.';return;}
  await stopBarcodeScanner();
  try{const formats=typeof Html5QrcodeSupportedFormats!=='undefined'?[Html5QrcodeSupportedFormats.EAN_13,Html5QrcodeSupportedFormats.EAN_8,Html5QrcodeSupportedFormats.UPC_A,Html5QrcodeSupportedFormats.UPC_E,Html5QrcodeSupportedFormats.CODE_128,Html5QrcodeSupportedFormats.CODE_39,Html5QrcodeSupportedFormats.QR_CODE]:undefined;barcodeScanner=new Html5Qrcode('barcodeReader',formats?{formatsToSupport:formats,verbose:false}:{verbose:false});await barcodeScanner.start({facingMode:'environment'},{fps:10,qrbox:{width:260,height:150},aspectRatio:1.5},code=>handleScannedCode(code),()=>{});$('#scannerStatus').textContent='Barcode mittig und ruhig vor die Kamera halten.';}catch(err){console.warn(err);$('#scannerStatus').textContent='Kamera nicht verfügbar. Auf dem iPhone ist dafür die private HTTPS-Adresse erforderlich; alternativ Code unten eingeben.';}
}
async function handleScannedCode(raw){const code=String(raw||'').trim();if(!code)return;await stopBarcodeScanner();if(scannerTarget==='item'){const input=$('#itemForm').elements.barcode;input.value=code;$('#scannerDialog').close();toast('Barcode in Artikel übernommen');return;}scannedItem=state.items.find(i=>i.barcode===code||i.sku===code);$('#scannerStatus').textContent=`Erkannt: ${code}`;$('#scannerResult').hidden=false;$('#scannerActions').hidden=false;if(scannedItem){$('#scannerResult').innerHTML=`<b>${esc(scannedItem.name)}</b><small>${esc([scannedItem.sku,scannedItem.color,scannedItem.size].filter(Boolean).join(' · '))} · Bestand ${number(scannedItem.stock)}</small>`;$('#scanPurchase').hidden=false;$('#scanSale').hidden=false;$('#scanEdit').textContent='Artikel öffnen';}else{$('#scannerResult').innerHTML=`<b>Unbekannter Barcode</b><small>${esc(code)} ist noch keinem Artikel zugeordnet.</small>`;$('#scanPurchase').hidden=true;$('#scanSale').hidden=true;$('#scanEdit').textContent='Neuen Artikel anlegen';$('#scanEdit').dataset.code=code;}}
...
$('#barcodeImage').onchange=async e=>{const file=e.target.files[0];if(!file)return;try{await stopBarcodeScanner();barcodeScanner=new Html5Qrcode('barcodeReader',{verbose:false});$('#scannerStatus').textContent='Bild wird ausgewertet …';const code=await barcodeScanner.scanFile(file,true);await handleScannedCode(code);}catch(err){console.warn(err);$('#scannerStatus').textContent='Auf diesem Foto wurde kein unterstützter Barcode erkannt. Bitte näher und schärfer fotografieren.';}e.target.value='';};
...
$('#scanEdit').onclick=()=>{const code=$('#scanEdit').dataset.code||$('#manualBarcode').value;if($('#scannerDialog').open)$('#scannerDialog').close();if(scannedItem)openItem(scannedItem.id);else{openItem();$('#itemForm').elements.barcode.value=code;}};
...
function eanCheckDigit(base){return String((10-[...base].reduce((sum,d,i)=>sum+(+d)*(i%2?3:1),0)%10)%10);}
...
$('#downloadBarcodePng').onclick=async()=>{try{const data=currentItemBarcodeExport(),cfg={width:80,height:32,dpi:300,background:'#ffffff'},blob=await svgToPngBlob(data.svg,cfg),a=document.createElement('a'),url=URL.createObjectURL(blob);a.href=url;a.download=`Barcode-${data.name}-300dpi.png`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1500);toast('Barcode als PNG heruntergeladen');}catch(err){alert(err.message||err);}};
...
function buildFullLabelSvg(item,cfg=labelConfig()){const W=Math.round(cfg.width*10),H=Math.round(cfg.height*10),code=ensureItemBarcode(item),barcode=barcodeVector(code,cfg.format,W*.05,H*.27,W*.9,H*.56),nameSize=Math.max(16,Math.min(32,H*.105))*cfg.fontScale,metaSize=Math.max(11,Math.min(20,H*.06))*cfg.fontScale,meta=[];if(cfg.showSku&&item.sku)meta.push(item.sku);if(cfg.showVariant)meta.push([item.color,item.size].filter(Boolean).join(' / '));if(cfg.showPrice&&item.salePrice)meta.push(money(item.salePrice));const logo=cfg.logo?`<image href="${xmlEscape(cfg.logo)}" x="${W*.03}" y="${H*.035}" width="${W*.16}" height="${H*.18}" preserveAspectRatio="xMidYMid meet"/>`:'';const nameX=cfg.logo?W*.57:W*.5,nameWidth=cfg.logo?W*.72:W*.94;return{format:barcode.actual,svg:`<svg xmlns="http://www.w3.org/2000/svg" width="${cfg.width}mm" height="${cfg.height}mm" viewBox="0 0 ${W} ${H}" role="img" aria-label="${xmlEscape(item.name)}"><rect width="${W}" height="${H}" rx="8" fill="${xmlEscape(cfg.background)}"/>${logo}${cfg.showName?`<text x="${nameX}" y="${H*.17}" text-anchor="middle" fill="${xmlEscape(cfg.textColor)}" font-family="Arial,Helvetica,sans-serif" font-size="${nameSize}" font-weight="700" textLength="${nameWidth}" lengthAdjust="spacingAndGlyphs">${xmlEscape(item.name)}</text>`:''}${barcode.svg}<text x="${W/2}" y="${H*.94}" text-anchor="middle" fill="${xmlEscape(cfg.textColor)}" font-family="Arial,Helvetica,sans-serif" font-size="${metaSize}">${xmlEscape(meta.filter(Boolean).join(' · '))}</text></svg>`};}
function saveLabelStyle(cfg){state.settings.labelStyle={background:cfg.background,textColor:cfg.textColor,dpi:cfg.dpi,fontScale:cfg.fontScale};try{localStorage.setItem(STORAGE_KEY,JSON.stringify(state));}catch{}}
function renderLabelPreview(){const items=selectedLabelItems(),item=items[0];$('#labelItemWrap').hidden=$('#labelScope').value!=='single';$('#customLabelSize').hidden=$('#labelPreset').value!=='custom';if(!item){$('#labelPreview').innerHTML='';$('#labelStatus').textContent='Keine Artikel in dieser Auswahl.';return;}const cfg=labelConfig(),built=buildFullLabelSvg(item,cfg);$('#labelPreview').innerHTML=built.svg;$('#labelStatus').textContent=`${items.length} Artikel · ${cfg.copies} Etikett(en) je Artikel · ${built.format} · ${cfg.dpi} DPI`;saveLabelStyle(cfg);}
...
$('#labelLogoInput').onchange=async e=>{const file=e.target.files[0];if(!file)return;try{let data;if(file.type==='image/svg+xml'||/\.svg$/i.test(file.name)){const doc=new DOMParser().parseFromString(await file.text(),'image/svg+xml');if(doc.querySelector('parsererror'))throw Error('Ungültige SVG-Datei');doc.querySelectorAll('script,style,foreignObject,iframe,object,embed').forEach(n=>n.remove());doc.querySelectorAll('*').forEach(n=>[...n.attributes].forEach(a=>{if(/^on/i.test(a.name)||(/url\s*\(/i.test(a.value))||((a.name==='href'||a.name==='xlink:href')&&!/^(#|data:)/i.test(a.value)))n.removeAttribute(a.name);}));data=`data:image/svg+xml;charset=utf-8,${encodeURIComponent(new XMLSerializer().serializeToString(doc.documentElement))}`;}else data=await fileToDataUrl(file);state.settings.labelLogo=data;try{localStorage.setItem(STORAGE_KEY,JSON.stringify(state));}catch{throw Error('Logo ist für den lokalen Speicher zu groß');}updateLogoPreview();renderLabelPreview();}catch(err){alert(`Logo konnte nicht importiert werden: ${err.message||err}`);}e.target.value='';};
$('#removeLabelLogo').onclick=()=>{state.settings.labelLogo='';try{localStorage.setItem(STORAGE_KEY,JSON.stringify(state));}catch{}updateLogoPreview();renderLabelPreview();};
$('#inventoryLabels').onclick=$('#settingsLabels').onclick=()=>openLabelStudio('','all');
$('#labelCurrentItem').onclick=()=>{const id=$('#itemForm').elements.id.value;if(!id){toast('Artikel zuerst speichern');return;}openLabelStudio(id,'single');};
...
function inventoryMatrix(){const head=['Artikel','SKU','Kategorie','Farbe','Größe','Marke','Material','Saison','Einheit','Tags','Beschreibung','Lieferant','Lagerplatz','Barcode','Bestand','Mindestbestand','EK','VK','Archiviert'];const rows=state.items.map(i=>[i.name,i.sku,i.category,i.color,i.size,i.brand,i.material,i.season,i.unit,i.tags,i.description,i.supplier,i.location,i.barcode,i.stock,i.minStock,i.cost,i.salePrice,i.archived?'Ja':'Nein']);return{head,rows};}
...
$('#exportXml').onclick=()=>{const fields=['name','sku','category','color','size','brand','material','season','unit','tags','description','supplier','location','barcode','stock','minStock','cost','salePrice','archived'],xml=`<?xml version="1.0" encoding="UTF-8"?>\n<hp67Inventory version="2" exportedAt="${new Date().toISOString()}">\n  <items>\n${state.items.map(i=>`    <item id="${xmlEscape(i.id)}">\n${fields.map(f=>`      <${f}>${xmlEscape(i[f]??'')}</${f}>`).join('\n')}\n    </item>`).join('\n')}\n  </items>\n</hp67Inventory>`;download(`HP67-Inventar-${new Date().toISOString().slice(0,10)}.xml`,'application/xml;charset=utf-8',xml);};
$('#exportTransactions').onclick=()=>{const head=['Datum','Art','Artikel','Menge','Stückpreis','Einstand','Notiz'];const rows=state.transactions.slice().sort((a,b)=>b.date.localeCompare(a.date)).map(t=>[fmtDate(t.date),t.type,t.itemName,t.quantity,t.unitPrice,t.unitCost??'',t.note]);download(`HP67-Buchungen-${new Date().toISOString().slice(0,10)}.csv`,'text/csv;charset=utf-8','\ufeff'+[head,...rows].map(r=>r.map(csvCell).join(';')).join('\r\n'));};
$('#exportXlsx').onclick=()=>{if(typeof XLSX==='undefined'){alert('Excel-Modul konnte nicht geladen werden.');return;}const wb=XLSX.utils.book_new();const inventory=state.items.map(i=>({Artikel:i.name,SKU:i.sku,Kategorie:i.category,Farbe:i.color,Größe:i.size,Marke:i.brand,Material:i.material,Saison:i.season,Einheit:i.unit,Tags:i.tags,Beschreibung:i.description,Lieferant:i.supplier,Lagerplatz:i.location,Barcode:i.barcode,Bestand:i.stock,Mindestbestand:i.minStock,EK:i.cost,VK:i.salePrice,Warenwert:i.stock*i.cost,Archiviert:i.archived?'Ja':'Nein'}));const tx=state.transactions.map(t=>({Datum:fmtDate(t.date),Art:t.type,Artikel:t.itemName,Menge:t.quantity,Stückpreis:t.unitPrice,Einstand:t.unitCost??'',Notiz:t.note}));const orders=state.orders.flatMap(o=>o.lines.map(l=>({Datum:fmtDate(o.date),Lieferant:o.supplier,Status:orderStatusLabel(o.status),Artikel:l.itemName,Bestellt:l.quantity,Erhalten:l.received||0,Offen:Math.max(0,l.quantity-(l.received||0)),EK:l.unitCost})));const snapshots=state.snapshots.map(s=>({Datum:fmtDate(s.date),Artikel:s.itemCount,Teile:s.totalUnits,Warenwert:s.value}));XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(inventory),'Inventar');XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(tx),'Buchungen');XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(orders),'Bestellungen');XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(snapshots),'Momentaufnahmen');XLSX.writeFile(wb,`HP67-Komplett-${new Date().toISOString().slice(0,10)}.xlsx`,{compression:true});};
...
$('#importJson').onchange=async e=>{try{const file=e.target.files[0];if(!file)return;const data=JSON.parse(await file.text());if(!Array.isArray(data.items)||!Array.isArray(data.transactions))throw Error();if(!confirm(`Sicherung mit ${data.items.length} Artikeln laden? Der aktuelle Stand wird ersetzt.`))return;data.orders=Array.isArray(data.orders)?data.orders:[];data.snapshots=Array.isArray(data.snapshots)?data.snapshots:[];data.categories=Array.isArray(data.categories)?data.categories:DEFAULT_CATEGORIES.map(c=>({...c}));data.brands=Array.isArray(data.brands)?data.brands:DEFAULT_BRANDS.slice();data.materials=Array.isArray(data.materials)?data.materials:DEFAULT_MATERIALS.slice();data.settings=data.settings&&typeof data.settings==='object'?data.settings:{};data.items.forEach(i=>{if(typeof i.archived!=='boolean')i.archived=false;});state=data;data.items.forEach(i=>{ensureCategory(i.category);ensureBrand(i.brand);ensureMaterial(i.material);});save();toast('Sicherung geladen');}catch{alert('Diese Datei ist keine gültige HP67-Sicherung.');}e.target.value='';};
function normalizeImportRows(rows){
  ...
  const col={name:ix('artikel','name','artikelname'),sku:ix('sku','artikelnummer'),category:ix('kategorie'),color:ix('farbe'),size:ix('grosse','groesse'),brand:ix('marke','brand','hersteller'),material:ix('material'),season:ix('saison','kollektion'),unit:ix('einheit','unit'),tags:ix('tags','schlagworte'),description:ix('beschreibung','hinweise','description'),supplier:ix('lieferant'),location:ix('lagerplatz','lagerort'),barcode:ix('barcode','ean'),stock:ix('bestand','menge'),minStock:ix('mindestbestand','minimum'),cost:ix('ek','einkaufspreis'),salePrice:ix('vk','verkaufspreis'),archived:ix('archiviert','archived')};if(col.name<0)throw Error('Spalte Artikel fehlt');
  const val=(r,index)=>index>=0?String(r[index]??'').trim():'';return rows.slice(1).filter(r=>val(r,col.name)).map(r=>({name:val(r,col.name),sku:val(r,col.sku),category:val(r,col.category),color:val(r,col.color),size:val(r,col.size),brand:val(r,col.brand),material:val(r,col.material),season:val(r,col.season),unit:val(r,col.unit)||'Stück',tags:val(r,col.tags),description:val(r,col.description),supplier:val(r,col.supplier),location:val(r,col.location),barcode:val(r,col.barcode),stock:col.stock>=0?parseNumber(r[col.stock]):0,minStock:col.minStock>=0?parseNumber(r[col.minStock]):5,cost:col.cost>=0?parseNumber(r[col.cost]):0,salePrice:col.salePrice>=0?parseNumber(r[col.salePrice]):0,archived:/^(ja|true|1)$/i.test(val(r,col.archived))}));
}
function parseInventoryXml(text){const doc=new DOMParser().parseFromString(text,'application/xml');if(doc.querySelector('parsererror'))throw Error('Ungültiges XML');const fields=['name','sku','category','color','size','brand','material','season','unit','tags','description','supplier','location','barcode','stock','minStock','cost','salePrice','archived'],headers={name:'Artikel',category:'Kategorie',color:'Farbe',size:'Größe',brand:'Marke',season:'Saison',unit:'Einheit',description:'Beschreibung',supplier:'Lieferant',location:'Lagerplatz',stock:'Bestand',minStock:'Mindestbestand',cost:'EK',salePrice:'VK',archived:'Archiviert'};const rows=[fields.map(f=>headers[f]||f)];doc.querySelectorAll('item').forEach(node=>rows.push(fields.map(f=>node.querySelector(f)?.textContent||'')));return normalizeImportRows(rows);}
function showImportPreview(incoming,fileName){if(!incoming.length)throw Error('Keine Artikel erkannt');pendingImportRows=incoming;$('#importSummary').textContent=`${incoming.length} Artikel aus „${fileName}“ erkannt. Die ersten Positionen werden unten angezeigt.`;$('#importPreviewRows').innerHTML=incoming.slice(0,12).map(i=>`<tr><td>${esc(i.name)}</td><td>${esc(i.sku)}</td><td>${number(i.stock)}</td><td>${money(i.cost)}</td></tr>`).join('');$('#importDialog').showModal();}
...
$('#importForm').addEventListener('submit',e=>{e.preventDefault();const strategy=e.currentTarget.elements.strategy.value;if(!pendingImportRows.length)return;if(strategy==='replace'){if(!confirm('Inventar vollständig ersetzen? Dabei werden auch bisherige Buchungen, Bestellungen und Momentaufnahmen gelöscht.'))return;state={items:pendingImportRows.map(data=>({id:uid(),photo:'',...data})),transactions:[],orders:[],snapshots:[],categories:DEFAULT_CATEGORIES.map(c=>({...c})),brands:DEFAULT_BRANDS.slice(),materials:DEFAULT_MATERIALS.slice(),settings:state.settings||{}};}else pendingImportRows.forEach(data=>{const existing=state.items.find(i=>(data.sku&&i.sku===data.sku)||(data.barcode&&i.barcode===data.barcode)||i.name.localeCompare(data.name,'de',{sensitivity:'base'})===0);if(existing&&strategy==='merge')Object.assign(existing,data);else if(!existing)state.items.push({id:uid(),photo:'',...data});});state.items.forEach(item=>{ensureCategory(item.category);ensureBrand(item.brand);ensureMaterial(item.material);});const count=pendingImportRows.length;pendingImportRows=[];save();$('#importDialog').close();toast(`${count} Importzeilen verarbeitet`);});
$('#resetData').onclick=()=>{if(confirm('Wirklich alle Artikel, Bestellungen, Momentaufnahmen und Buchungen unwiderruflich löschen?')){state={items:[],transactions:[],orders:[],snapshots:[],categories:DEFAULT_CATEGORIES.map(c=>({...c})),brands:DEFAULT_BRANDS.slice(),materials:DEFAULT_MATERIALS.slice(),settings:{}};save();toast('Alle Daten gelöscht');}};
