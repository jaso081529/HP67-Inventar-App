...
function inventoryMatrix(){const head=['Artikel','SKU','Kategorie','Farbe','Größe','Marke','Material','Saison','Einheit','Tags','Beschreibung','Verkaufsoptionen','Lieferant','Lagerplatz','Barcode','Bestand','Mindestbestand','EK','VK','Archiviert'];const rows=state.items.map(i=>[i.name,i.sku,i.category,i.color,i.size,i.brand,i.material,i.season,i.unit,i.tags,i.description,(i.salesOptions||[]).join(', '),i.supplier,i.location,i.barcode,i.stock,i.minStock,i.cost,i.salePrice,i.archived?'Ja':'Nein']);return{head,rows};}
...
function parseInventoryXml(text){const doc=new DOMParser().parseFromString(text,'application/xml');if(doc.querySelector('parsererror'))throw Error('Ungültiges XML');const fields=['name','sku','category','color','size','brand','material','season','unit','tags','description','salesOptions','supplier','location','barcode','stock','minStock','cost','salePrice','archived'],headers={name:'Artikel',category:'Kategorie',color:'Farbe',size:'Größe',brand:'Marke',season:'Saison',unit:'Einheit',description:'Beschreibung',salesOptions:'Verkaufsoptionen',supplier:'Lieferant',location:'Lagerplatz',stock:'Bestand',minStock:'Mindestbestand',cost:'EK',salePrice:'VK',archived:'Archiviert'};const rows=[fields.map(f=>headers[f]||f)];doc.querySelectorAll('item').forEach(node=>rows.push(fields.map(f=>node.querySelector(f)?.textContent||'')));return normalizeImportRows(rows);}
