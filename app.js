...
$('#itemForm').addEventListener('submit',e=>{
  ...
  const oldItem=existing?{...existing}:null,oldItems=state.items.slice(),oldCategories=state.categories.slice(),oldBrands=state.brands.slice(),oldMaterials=state.materials.slice();
  ensureCategory(item.category);
  ensureBrand(item.brand);
  ensureMaterial(item.material);
  let created=1;
  if(existing)Object.assign(existing,item);else if(batchMode){created=variantCount;for(const color of colors)for(const size of sizes){const variant={...item,id:uid(),color,size,sku:'',barcode:''};variant.sku=createTextileSku(variant);variant.barcode=createInternalEan();state.items.push(variant);}}else state.items.push(item);
  if(!persistState()){
    state.items=oldItems;if(existing)Object.assign(existing,oldItem);
...
function buildFullLabelSvg(item,cfg=labelConfig()){const W=Math.round(cfg.width*10),H=Math.round(cfg.height*10),code=ensureItemBarcode(item),nameSize=Math.max(16,Math.min(32,H*.105))*cfg.fontScale,metaSize=Math.max(11,Math.min(20,H*.06))*cfg.fontScale,meta=[];if(cfg.showSku&&item.sku)meta.push(item.sku);if(cfg.showVariant)meta.push([item.color,item.size].filter(Boolean).join(' / '));if(cfg.showPrice&&item.salePrice)meta.push(money(item.salePrice));const logo=cfg.logo?`<image href="${xmlEscape(cfg.logo)}" x="${W*.03}" y="${H*.035}" width="${W*.16}" height="${H*.18}" preserveAspectRatio="xMidYMid meet"/>`:'',nameX=cfg.logo?W*.59:W*.5,nameWidth=cfg.logo?W*.74:W*.9,maxChars=Math.max(10,Math.floor(nameWidth/(nameSize*.58))),nameLines=wrapLabelName(item.name,maxChars),twoLines=nameLines.length>1,barcodeY=twoLines?H*.34:H*.27,barcode=barcodeVector(code,cfg.format,W*.05,barcodeY,W*.9,twoLines?H*.49:H*.56),lineGap=nameSize*1.08,nameStart=twoLines?H*.095:H*.17,nameSvg=cfg.showName?`<text text-anchor="middle" fill="${xmlEscape(cfg.textColor)}" font-family="Arial,Helvetica,sans-serif" font-size="${nameSize}" font-weight="700">${nameLines.map((line,index)=>`<tspan x="${nameX}" y="${nameStart+index*lineGap}">${xmlEscape(line)}</tspan>`).join('')}</text>`:'';return{format:barcode.actual,svg:`<svg xmlns="http://www.w3.org/2000/svg" width="${cfg.width}mm" height="${cfg.height}mm" viewBox="0 0 ${W} ${H}" role="img" aria-label="${xmlEscape(item.name)}"><rect width="${W}" height="${H}" rx="8" fill="${xmlEscape(cfg.background)}"/>${logo}${nameSvg}${barcode.svg}<text x="${W/2}" y="${H*.94}" text-anchor="middle" fill="${xmlEscape(cfg.textColor)}" font-family="Arial,Helvetica,sans-serif" font-size="${metaSize}">${xmlEscape(meta.filter(Boolean).join(' · '))}</text></svg>`};}
...
function renderLabelPreview(){const scope=$('#labelScope').value,items=selectedLabelItems(),item=items[0];$('#labelItemWrap').hidden=scope!=='single';$('#labelVariantWrap').hidden=scope!=='variants';$('#customLabelSize').hidden=$('#labelPreset').value!=='custom';if(!item){$('#labelPreview').innerHTML='';$('#labelStatus').textContent='Keine Variante ausgewählt.';return;}const cfg=labelConfig(),built=buildFullLabelSvg(item,cfg);$('#labelPreview').innerHTML=built.svg;$('#labelStatus').textContent=`${items.length} Variante(n) · ${cfg.copies} Etikett(en) je Variante · ${built.format} · ${cfg.dpi} DPI`;saveLabelStyle(cfg);}
